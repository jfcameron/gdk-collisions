...
namespace {
    ...
    [[nodiscard]] world make_world(const task_dispatcher_type &aDispatcher) {
        world result;
        result.scene = impl_collision_scene::make(nullptr, nullptr, impl_collision_policy{}, aDispatcher);
        result.ground = result.scene->make_static_plane_collider(matrix4x4_type::identity);
    ...
    [[nodiscard]] std::vector<vector3_type> run(const task_dispatcher_type &aDispatcher) {
    ...
    [[nodiscard]] bool identical(const std::vector<vector3_type> &aFirst,
        const std::vector<vector3_type> &aSecond) {
    ...
    [[nodiscard]] bool moved(const std::vector<vector3_type> &aPositions) {
        return !aPositions.empty()
            && std::any_of(aPositions.begin(), aPositions.end(),
                [](const vector3_type &aPosition) { return aPosition.x != 37.0f; });
    }
}

TEST_CASE("gdk::scene gives the same result with a dispatcher as without one",
    "[gdk::collision]") {
    const auto serial = run({});

    const auto dispatched = run([](std::size_t aCount, const chunk_type &aChunk) {
...
TEST_CASE("gdk::scene is unaffected by the order chunks run in", "[gdk::collision]") {
    const auto serial = run({});
    REQUIRE(moved(serial));

    SECTION("reversed") {
        const auto reversed = run([](std::size_t aCount, const chunk_type &aChunk) {
    ...
    SECTION("odd chunks before even ones") {
        const auto interleaved = run([](std::size_t aCount, const chunk_type &aChunk) {
...
TEST_CASE("gdk::scene answers raycasts from several threads at once", "[gdk::collision]") {
    auto w = make_world({});
...
TEST_CASE("gdk::scene gives the same result on several threads", "[gdk::collision]") {
    const auto serial = run({});
    REQUIRE(moved(serial));

    const auto threaded = [](std::size_t aCount, const chunk_type &aChunk) {
        std::vector<std::thread> workers;
