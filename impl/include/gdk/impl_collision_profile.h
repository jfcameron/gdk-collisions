...
#ifndef GDK_IMPL_COLLISION_PROFILE_H
#define GDK_IMPL_COLLISION_PROFILE_H
...
#define GDK_COLLISION_PROFILE_END(FIELD) \
    ::gdk::collision_profile::accumulated().FIELD += ::std::chrono::duration<double, ::std::milli>( \
        ::std::chrono::steady_clock::now() - gdkCollisionProfileStart_##FIELD).count()

#define GDK_COLLISION_PROFILE_COUNT_STEP() ++::gdk::collision_profile::accumulated().steps

#define GDK_COLLISION_PROFILE_COUNT(FIELD, AMOUNT) \
    ::gdk::collision_profile::accumulated().FIELD += (AMOUNT)
