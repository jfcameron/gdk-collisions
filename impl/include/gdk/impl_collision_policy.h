...
#ifndef GDK_IMPL_COLLISION_POLICY_H
#define GDK_IMPL_COLLISION_POLICY_H
...
namespace gdk {
    ...
    struct impl_collision_policy final {
        ...
        const collision_delta_time_type TIME_OF_IMPACT_EPSILON{1e-7f};
        ...
        const collision_floating_point_type BROAD_PHASE_CELL_SIZE{2.0f};
        ...
        const collision_floating_point_type AXIS_ENTRY_EPSILON{1e-5f};

        /// \brief below this speed a body is treated as stationary and unswept tests are used.
        const collision_floating_point_type MIN_SWEPT_SPEED{1e-3f};

        /// \brief MIN_SWEPT_SPEED squared, for comparing against length_squared() and dot(v,v)
        const collision_floating_point_type MIN_SWEPT_SPEED_SQUARED{1e-6f};

        /// \brief minimum penetration depth
        const collision_floating_point_type PENETRATION_DEPTH_EPSILON{1e-6f};

        /// \brief used to determine when a vector is too small to reliably normalize
        const collision_floating_point_type NORMALIZATION_THRESHOLD{1e-6f};

        /// \brief how close |cos| between two clip planes must be to 1 for them to count as parallel
        const collision_floating_point_type CLIP_PLANE_PARALLEL_COSINE{0.9999995f};

        /// \brief how close to a triangle's edge a contact point must be to count as *on* it.
        const collision_floating_point_type BARYCENTRIC_EDGE_EPSILON{1e-4f};

        /// \brief how nearly a contact normal must match a face normal to count as a face contact.
        const collision_floating_point_type FACE_CONTACT_COSINE_EPSILON{1e-4f};

        /// \brief minimum offset applied after collision resolution
        const collision_floating_point_type POST_RESOLUTION_SEPARATION_EPSILON{1e-3f};
    };
}
