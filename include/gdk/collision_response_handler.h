...
#ifndef GDK_COLLISION_RESPONSE_HANDLER_H
#define GDK_COLLISION_RESPONSE_HANDLER_H
...
namespace gdk {
    ...
    using collision_clip_planes_type = std::vector<collision_vector3_type>;
    ...
    struct contact_context final {
        ...
        collision_vector3_type collision_normal;

        /// \brief the time this handler's answer will act over
        collision_delta_time_type remaining_time;

        /// \brief surfaces already redirected along this step
        const collision_clip_planes_type &clip_planes;
    ...
    using collision_response_handler = std::function<
        collision_vector3_type ( //velocity delta
            collider&, //aThis
    ...
    namespace collision_response_handlers {
        /// \brief velocity is not altered at all after overlap resolution
        inline const collision_response_handler null_opt = [](collider&, const contact_context&){
            return collision_vector3_type::zero;
        };

        /// \brief redirect along the surface then restore the original speed
        [[nodiscard]] const collision_response_handler &slide_preserving_speed();
        ...
        [[nodiscard]] const collision_response_handler &slide_projecting();
    }
}
